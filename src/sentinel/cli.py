...
def _emit(result, output_path):
    """Write report JSON to file or stdout."""
    import json
...
@click.option("--rps", default=1.0, type=float,
              help="Max requests per second against target (politeness throttle).")
...
def main(target: str, base_url: str, mode: str,
         max_endpoints: int, interval: int,
         rps: float, output: str | None) -> None:
